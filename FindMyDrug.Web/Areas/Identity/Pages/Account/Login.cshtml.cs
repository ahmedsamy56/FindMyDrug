...
namespace FindMyDrug.Web.Areas.Identity.Pages.Account
{
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            if (User.Identity.IsAuthenticated) // Check if the user is already authenticated
            {
                ...
                if (await _signInManager.UserManager.IsInRoleAsync(user, SD.AdminRole))
                {
                    ...
                    return; // End the execution after redirecting
                }
                else if (await _signInManager.UserManager.IsInRoleAsync(user, SD.pharmaceuticalRole))
                {
                    Response.Redirect("/Pharmaceutical/Home");
                    return; // End the execution after redirecting
                }

                // Redirect to a default page if no specific role found
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        return LocalRedirect("/Pharmaceutical/Home");
                    }
